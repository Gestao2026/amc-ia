...
Solução: encurtados os nomes repetidos da árvore de classificação, dentro de `02 - Editais Abertos`, em 36 pastas: `03 - Fundos (Criança, Idoso e Saúde)` virou `03 - Fundos` (ganho de 25), `01 - Com ou Sem Fins` e `01 - Com ou sem fins lucrativos` viraram `01 - Ambos` (10 e 21), `02 - Sem fins lucrativos` virou `02 - Sem Fins` (11), `03 - Com fins lucrativos` virou `03 - Com Fins` (11) e `06 - Natureza não informada` virou `06 - Sem info` (14). A pasta que travava caiu de 243 para 208 caracteres (folga de 15 para 50) e as pastas apertadas caíram de 33 para 8. Nenhum arquivo foi renomeado nem movido.

Duas convenções conviviam na mesma árvore, e isso só apareceu no meio da execução: a antiga, abreviada (`01 - Ambos`, `02 - Sem Fins`), e a nova das subárvores `07 - Fundos Públicos` e `08 - Fundos Privados`, por extenso (`01 - Com ou sem fins lucrativos`, com 31 caracteres). A convenção por extenso já tinha produzido uma pasta estourada (263 caracteres, folga negativa) na LMIC Multilinguagens. Padronizada na abreviada. Ficaram 8 pastas em `06 - Sem informação` contra 7 em `06 - Sem info`, diferença sem efeito prático (todas com folga acima de 44) e deixada para a captadora decidir.
...
Três pastas devolveram `acesso negado` na primeira passada, e duas delas passaram sozinhas numa tentativa posterior. Não é permissão (os atributos são normais), é handle aberto: a captadora estava trabalhando na árvore no mesmo momento, salvando editais e reorganizando pastas, e uma janela do Explorer posicionada em uma pasta trava a renomeação dela e de todas as pastas acima. Registro para não reinvestigar: **renomeação de pasta em massa deve ser feita com o Explorer fora da árvore**, o `acesso negado` nesse contexto quase nunca é ACL, e vale repetir a tentativa antes de concluir que está bloqueado.

Achado colateral, sem relação com o limite de caracteres, mas que quebrava automação em silêncio: a captadora renumerou as pastas de primeiro nível da Mineração, e `04 - Histórico de Editais _ Não Submetidos` virou `06 - ...`. O `/editais-pasta-processar` (Passo 4.1, destino fixo dos editais vencidos do SOL-0011) e a memória `reference_pasta_matriz_editais` ainda apontavam para o `04`, ou seja, o fluxo de editais vencidos teria falhado ou criado a pasta errada na próxima execução. Os dois foram corrigidos. Lição: **caminho externo fixado em comando ou memória precisa ser reconferido sempre que se mexer nessa árvore**, porque a captadora reorganiza essas pastas com frequência e nada avisa quando a referência quebra.
...
### SOL-0027. Documento sai também em PDF, e a limpeza dos caminhos longos fora da Mineração

Problema: dois pedidos da captadora na mesma frase, depois de ela instalar o PDF24 Creator. Primeiro, os documentos gerados aqui (Mapa do Edital, Checklist Mobilizando e os três modelos de análise) só existiam em `.docx`, então quem recebe precisa do Word para abrir. Segundo, continuavam 222 arquivos da pasta `_82` acima do limite de 259 caracteres, o problema do SOL-0021, que o SOL-0023 tinha começado a tratar só na árvore da Mineração de Editais.

Solução, parte 1 (PDF). Criado `scripts/docx-para-pdf.ps1`, que converte pelo próprio Word (o PDF24 não serve: `pdf24-DocTool.exe` não tem linha de comando, abre janela, e nenhum executável dele declara suporte a caminho longo no manifesto). `scripts/mapa-edital.py` passou a chamar esse conversor ao fim da geração, então os dois documentos saem sempre nos dois formatos, com `--sem-pdf` para desligar. Os três modelos ganharam a versão PDF versionada.

Dois cuidados no conversor, que não devem ser removidos: ele detecta se o Word já estava aberto e, nesse caso, **nunca** chama `Quit` (o COM do Word é instância única por usuário, e sair fecharia os documentos que ela está usando); e confere o PDF depois de gravar, em vez de confiar na ausência de erro, pela mesma disciplina do SOL-0014.

Defeito real encontrado no caminho: `scripts/exportar-projeto.py` procurava o Chrome em `/Applications/Google Chrome.app/...`, caminho de macOS. Nesta máquina a checagem sempre falhava, então `proposta.pdf`, `orcamento.pdf` e `projeto-completo.pdf` **nunca foram gerados**, e o comando caía sempre no aviso de "abra o `-impressao.html` e imprima à mão", que parecia comportamento normal. Corrigido com uma lista de navegadores que cobre Chrome e Edge no Windows, mais `Path.as_uri()` para montar o endereço do arquivo: sem isso, espaço e acento no caminho fazem o navegador não achar o arquivo, imprimir a própria página de erro e ainda assim devolver um PDF, que passaria por sucesso (a mesma armadilha do SOL-0021).

Solução, parte 2 (caminhos longos). Criados dois utilitários reaproveitáveis, porque isso vai se repetir: `scripts/encurtar-caminhos.ps1` (renomeia pastas a partir de um plano em JSON, da mais funda para a mais rasa, confere depois e grava registro de reversão) e `scripts/encurtar-nomes-arquivo.ps1` (último recurso, encurta o nome do arquivo, aceitando uma lista de nomes escolhidos a dedo). Aplicados em três rodadas de pasta mais uma de nome: **222 arquivos fora do limite passaram a 37**, e os 37 restantes são todos da árvore da Mineração, que estava sendo mexida por outra conversa ao mesmo tempo e por isso ficou de fora de propósito. Conferido no fim com o teste de abertura real do SOL-0021 (Edge em modo headless, lendo o texto da saída atrás de `ERR_FILE_NOT_FOUND`), nos cinco arquivos mais longos que sobraram: todos abrem.

Ordem que se confirmou certa, e que vale para a próxima vez: encurtar a árvore de pastas primeiro (SOL-0023), porque o corte automático de nome de arquivo destrói justamente o que identifica o documento. No primeiro passe, com as pastas ainda longas, a poda automática ia transformar `certidao_de_aprovacao_assinada-20250325091836-2024.08.0514-4125-4523.pdf` em `certidao_de_aprovacao_assinada.pdf`, jogando fora o número do processo e deixando quatro arquivos diferentes com nomes quase iguais. Depois de encurtar as pastas, sobraram 21 arquivos, tratados um a um com nome escolhido (o mesmo caso virou `certidao_aprovacao-2024.08.0514-4125-4523.pdf`, que preserva o processo e joga fora só o carimbo de hora da máquina).

Armadilha nova, e que explica uma falha silenciosa difícil de enxergar: **o PowerShell 5.1 lê arquivo `.ps1` sem BOM como ANSI, não como UTF-8**. Um script escrito com acento no corpo (por exemplo o filtro `"01 - Mineração de Editais"` ou uma lista de nomes de arquivo com acento) compara contra texto corrompido e simplesmente não encontra nada, sem erro nenhum. Foi exatamente o que aconteceu: a lista de nomes escolhidos casou zero itens acentuados, e o filtro de pasta a ignorar deixou passar os 37 arquivos da Mineração. Regra que passa a valer: **texto acentuado deste projeto vive em JSON lido com `-Encoding UTF8` explícito, nunca escrito dentro de um `.ps1`**; as mensagens dos scripts em PowerShell continuam sem acento de propósito.

Efeito na sincronização da `_82`: 538 dos 2.535 arquivos do manifesto (21,2%) mudaram de caminho. Fica abaixo do gatilho de 30% da trava de exclusão em massa (SOL-0019), então a sincronização seguinte apaga sozinha os caminhos antigos no Drive e sobe os novos. Atenção para a próxima vez: a outra conversa estava encurtando a árvore da Mineração ao mesmo tempo, e as duas somadas chegariam perto do gatilho. Duas frentes de renomeação em massa no mesmo dia precisam ser medidas juntas, não cada uma por si.

Registros de reversão de todas as rodadas em `minhas-oscs/_carteira/caminhos-longos/` (fora do controle de versão, porque nomeiam clientes reais).

Alternativas descartadas: usar o PDF24 para converter (sem linha de comando utilizável) ou como leitor que alcance caminho longo (o manifesto dos executáveis não declara suporte, então não resolveria os 222); renomear a pasta raiz `_82 - Rosepaula Aparecida Andrade Rodrigues` para `_82`, que economizaria 40 caracteres de uma vez (recusada por ela no SOL-0021 e de novo no SOL-0023, e não foi trazida de volta); aceitar a poda automática de nome em tudo, que era mais rápida e teria estragado nome de documento real.

Data: 2026-08-14

### SOL-0028. Push automático das 02h religado (reverte o SOL-0022, restaura o SOL-0017)

Problema: a captadora perguntou, em 16/08/2026, se o GitHub `Gestao2026/amc-ia` estava mesmo recebendo as atualizações, porque outra pessoa (Deivid) tinha dito que "não funciona porque ele está público e vai direto para o github dele". Verificação ao vivo (`git remote -v`, `git fetch`, `gh repo view` via API pública) confirmou: o repositório é dela (dono `Gestao2026`), não é fork de nada (`fork: false`, sem `parent`), e vinha recebendo e gravando as atualizações normalmente até 12/08/2026 às 02h05, quando a publicação automática ainda existia. A afirmação do Deivid não correspondia ao estado real do repositório. O motivo de não haver nada novo publicado desde então era simplesmente a decisão dela própria no SOL-0022 (excluir o push automático em 13/08), somada ao bloqueio de conteúdo suspeito da varredura em 15/08 (CNPJ e e-mails de terceiro), ainda sem revisão. Diante disso, ela pediu: "pode fazer as atualizações no github gestao2026 automaticamente".

Solução: recriada a tarefa agendada `AMC-IA-Push-Diario` (`Register-ScheduledTask`, apagada no SOL-0022), com a mesma configuração original do SOL-0017: dispara às 02h todo dia, desperta o computador (`WakeToRun`), roda mesmo na bateria (`DisallowStartIfOnBatteries` e `StopIfGoingOnBatteries` em `False`, mesmo padrão já usado em `AMC-IA-Sincronizar-Pasta82`), chama `scripts/push-diario-seguro.ps1` em modo normal. A varredura de conteúdo (CPF, CNPJ, token, e-mail de terceiro, identificador de pasta do Drive, arquivo protegido e o texto dentro de `.docx`/`.xlsx`, ver SOL-0017 e SOL-0026) **não foi tocada nem enfraquecida**: o pedido dela foi por automação, não por desligar a proteção, e a proteção existe justamente porque o repositório é público. O script e o hook `post-commit` (que só avisa, não publica) continuam exatamente como estavam.

Consequência prática que ela precisa saber: o bloqueio de 15/08/2026 (CNPJ e e-mails de terceiro nas alterações pendentes) continua de pé. A tarefa recriada roda pela primeira vez às 02h de 17/08/2026, vai reavaliar o mesmo conteúdo e deve bloquear de novo pelo mesmo motivo, até que aquele achado seja revisado e resolvido (ou o conteúdo saia do que está pendente de publicar).

Alternativas descartadas: desligar a varredura de conteúdo para garantir que a publicação sempre aconteça (foi oferecida como opção explícita e desaconselhada antes do pedido dela; ela não pediu isso, só pediu "automático", então a leitura mais fiel ao pedido é manter a proteção e só devolver a automação); não mexer em nada e só responder a pergunta sobre o Deivid (ela já tinha pedido a automação antes, numa mensagem anterior, então a ação era esperada, não apenas informativa).
...
### SOL-0031. Trabalho na nuvem: a guarda não roda lá, e a publicação das 02h quebraria na primeira volta

Problema: o projeto passou a ser trabalhado também em sessões na nuvem (Claude Code na web), não só no computador da captadora. Ela pediu a checagem de conflitos entre as duas frentes antes de deixar isso virar rotina. A verificação achou quatro coisas, três delas defeitos reais:

1. **A guarda de conteúdo não existe na nuvem.** Toda a proteção dos SOL-0017, SOL-0026 e SOL-0030 (varrer CPF, CNPJ, token, e-mail de terceiro, dado real de cliente e o texto dentro de `.docx` e `.xlsx` antes de publicar) vive no `push-diario-seguro.ps1` e no hook `post-commit`. No ambiente da nuvem, `core.hooksPath` não está configurado, `.git/hooks/post-commit` não existe e não há PowerShell instalado, então nem o hook dispara nem o script rodaria. Ou seja, um envio feito direto da nuvem para a `main` chegaria a um repositório público sem passar por varredura nenhuma. O risco é atenuado porque `minhas-oscs/` real e o `.env` não existem naquela cópia (ambos no `.gitignore`), mas qualquer conteúdo colado na conversa e salvo em arquivo passaria batido.
2. **A publicação das 02h quebraria na primeira vez que algo da nuvem entrasse na `main`.** O `push-diario-seguro.ps1` fazia `git push origin main` sem nenhum `git fetch` ou `git pull` antes, e comparava `origin/main..HEAD` contra uma cópia local desatualizada do GitHub. Assim que a `main` do GitHub andasse por fora, o envio passaria a ser recusado por não ser avanço direto, gerando "FALHOU AO PUBLICAR" e alerta na Área de Trabalho todo dia, até alguém puxar na mão. Nada se perderia, mas a automação travaria em silêncio operacional (o alerta aparece, a causa não).
3. **Duas sessões na nuvem de 17/08/2026 ficaram com trabalho que nunca chegou ao GitHub.** `claude/atualizar-dashboard-projetos-phooae` (22h04, "dashboard atualizado com novo projeto") e `claude/sincronizacoes-atuais-6z7pb2` (23h17, mapeamento das sincronizações). Nenhum dos dois ramos existe no repositório remoto. Como o ambiente de cada sessão na nuvem é descartável, trabalho que não é enviado se perde quando o ambiente é reciclado.
4. **O `decisoes-tecnicas.md` é o ponto provável de colisão.** É o arquivo mais editado do projeto (22 dos commits dos últimos 30 dias, quase o dobro do segundo colocado), e os dois lados escrevem sempre no fim dele, que é exatamente o caso que o git não consegue juntar sozinho.

Solução, e o fluxo que passa a valer:

**A nuvem propõe, a captadora autoriza, o computador publica.** Nenhuma sessão na nuvem envia para a `main`. Ela sempre trabalha em ramo próprio, abre um pedido de aprovação no GitHub e para ali. A captadora aprova (ou não) na tela do GitHub, e o computador desce o que foi aprovado. Com isso, a guarda de conteúdo continua sendo o último filtro de tudo que é publicado a partir do computador, e o material vindo da nuvem passa por uma revisão humana explícita, que substitui a varredura ausente do ponto 1.

`scripts/push-diario-seguro.ps1` ganhou um passo 0, antes de qualquer comparação: confere se o repositório está na `main` (e para com aviso se não estiver, porque o commit da 01h30 grava no ramo aberto enquanto a publicação envia sempre a `main`), consulta o GitHub, e desce o que estiver novo. O caso comum, que é a captadora ter aprovado algo vindo da nuvem sem ter mexido no computador, resolve sozinho por avanço direto. Havendo trabalho dos dois lados, tenta juntar. Dando conflito de verdade, desfaz a junção, não publica e registra a pendência, para nunca deixar o repositório parado no meio de uma junção pela metade às duas da manhã.

Corrigido de passagem, no mesmo arquivo: o cabeçalho ainda dizia que a tarefa das 02h tinha sido apagada (SOL-0022), sem registrar que o SOL-0028 a recriou; e `AMC-IA-Push-Diario` continuava fora da lista de tarefas conferidas no relatório diário, pelo mesmo motivo desatualizado, de modo que uma falha da própria publicação deixava de ser reportada. Os dois voltaram ao estado correto.

Limite desta rodada, que precisa ficar visível: **o passo 0 foi escrito na nuvem e não pôde ser executado lá**, porque não existe PowerShell naquele ambiente. A conferência feita foi de leitura (balanceamento de blocos, ausência de caractere acentuado novo no `.ps1`, conforme o SOL-0027, e revisão do fluxo caso a caso). A validação de verdade é rodar `push-diario-seguro.ps1 -SoVerificar` no computador, que varre e relata sem publicar nada, antes de confiar na execução automática das 02h.

Alternativas descartadas: fazer a nuvem publicar direto na `main`, resolvendo o ponto 2 sem mexer no script (é justamente o que o ponto 1 proíbe, porque a guarda não roda lá e o repositório é público); usar `git pull --rebase` no passo 0 (reescreve o histórico local e, dando conflito no meio da madrugada, deixaria o repositório num estado de rebase pela metade, sem ninguém para resolver); portar a guarda de conteúdo para Python, para rodar também na nuvem (é a correção de fundo do ponto 1, mas duplica um detector que já é delicado e que teve dois defeitos reais no SOL-0026, e a revisão humana no GitHub cobre o risco enquanto isso; fica registrado como possibilidade se o volume de trabalho na nuvem crescer).

Impacto: regra que passa a valer. **Sessão na nuvem nunca envia para a `main`, sempre para um ramo com pedido de aprovação.** E, enquanto uma sessão na nuvem estiver com um SOL aberto, o computador não escreve outro, para não colidir no fim do mesmo arquivo. Ponto 3 sem solução automática: não há como uma sessão na nuvem alcançar outra sessão já encerrada (foi tentado, o ambiente aparece como fora de alcance), então trabalho de sessão na nuvem só se recupera reabrindo aquela sessão enquanto o ambiente dela ainda existe. Isso reforça a regra prática: trabalho feito na nuvem se envia no mesmo dia, ou se perde.
